...
{
    class TimeLimiter : IRateLimiter
    {
        readonly IAwaitableConstraint _AwaitableConstraint;
            ...
            static Func<Task> Transform(Action act)
            {
                return () => { act(); return Task.FromResult(0); };
            }

            static Func<Task<T>> Transform<T>(Func<T> compute)
            {
